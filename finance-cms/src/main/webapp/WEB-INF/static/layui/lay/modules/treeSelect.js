    ...
    TreeSelect.prototype.render = function(options) {
        ...
        settext = function(data, val, input) {
            ...
            function(_, o) {
                ...
                if (o.children) settext.call(this, o.children, val, input);
            });
        },
        ...
        events = function(reElem, treeid, disabled) {
            ...
            //如果控件有默认值，设置默认值
            if (defaultVal) settext.call(this, o.data, defaultVal, input);
            ...
            var showDown = function() {
                ...
                if (downHeight < 300) tree.css("max-height", downHeight + "px");
            ...
            layui.tree({
                ...
                click: function(obj) {
                    ...
                    hideDown(true);
                    if (o.selectcall) o.selectcall(obj);
                    return false;
                },
                nodes: o.data
            });
        ...
        init = function(treeid) {
            ...
            //替代元素
            var reElem = $(['<div class="layui-unselect ' + CLASS + (disabled ? ' layui-select-disabled': '') + '">', '<div class="' + TITLE + '"><input type="text" placeholder="' + placeholder + '" id="' + treeid + "_text" + '" value="' + (value ? selected.html() : '') + '" readonly class="layui-input layui-unselect' + (disabled ? (' ' + DISABLED) : '') + '">', '<i class="layui-edge"></i></div>', '<ul id="' + treeid + '" class="layui-anim layui-anim-upbit layui-tree"></ul>', '</div>'].join(''));
        ...
        if (!options) return reset.call(this, null);
        if (options.filter) return reset.call(this, options.filter);
        if (!othis[0]) return hint.error('layui.treeSelect 没有找到' + options.elem + '元素');
        if (!options.data) return hint.error('layui.treeSelect 缺少参数 data ,data 可直接指定treedata，也可以是treedata数据url，treedata参见layui tree模块');
